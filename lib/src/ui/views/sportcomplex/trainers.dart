...
class SportComplexTrainersView extends StatefulWidget implements ViewContract
{
    final RouterContract router;

    SportComplexTrainersView({Key key, this.router}): super(key: key);

    @override
    _SportComplexTrainerState createState() => _SportComplexTrainerState();
}

class _SportComplexTrainerState extends State<SportComplexTrainersView>
{
    ...
    Future<void> _loadData() async
    {
        if (_canLoad()) {
            ...
            try {
                List<Worker> workers = await _apiRepository.getSportComplexTrainers();
                _onLoad(workers);
            } catch(e) {
                _onError();
            }
